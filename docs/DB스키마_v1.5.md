...
> Uncounted Client — CTO 인수인계 문서 (2026-03-16)
> v1.5 — Migration 009~013 반영: session_chunks 신규, transcript_chunks.words 추가, session_chunks.labels 추가; 타입 오류 수정 (TEXT[] → JSONB); user_asset_ledger.unit_type 추가
...
│  │  session_chunks    │  transcript_chunks │              │
│  ├─────────────────────────────────────────────────────── │
...
| `component_ids` | JSONB | `["BASIC"]` | 컴포넌트 ID 목록 |
...
| `component_ids` | JSONB | `["BASIC"]` | 컴포넌트 목록 |
| `require_audio` | BOOLEAN | | 오디오 필수 |
| `require_labels` | JSONB | `false` | false \| true \| string[] (특정 필드명) |
...
| `domain_filter` | JSONB | `[]` | 도메인 제한 (빈 배열=전체) |
| `export_fields` | JSONB | `[]` | 내보내기 필드 |
...
| `selection_manifest` | JSONB | NULL | 선택된 BU ID 배열 (JSON array) |
...
> Migration 005에서 정의, Migration 006에서 unit_type 추가, Migration 008에서 확장 (META_EVENT_BASE + voided)
...
| `unit_type` | TEXT | 'AUDIO_BU' | 'AUDIO_BU' \| 'META_EVENT' (Migration 006) |
...
### session_chunks (WAV 물리 청크 업로드 이력)

> Migration 009에서 생성, 010에서 transcript_text 추가, 013에서 labels 추가.
> 대용량 WAV를 30초 단위로 분할 업로드할 때 각 청크의 메타데이터 저장.
...
| `storage_path` | TEXT | NOT NULL | sanitized-audio 버킷 경로. 형식: `{userId}/{sessionId}/{sessionId}-001.wav` |
...
| `file_size_bytes` | INTEGER | NULL | 업로드된 WAV 파일 크기(bytes) |
| `sample_rate` | INTEGER | 16000 | 샘플 레이트 |
| `upload_status` | TEXT | 'pending' | 'pending' \| 'uploaded' \| 'failed' |
| `transcript_text` | TEXT | NULL | STT 추출 후 PII 마스킹된 청크 텍스트 (Migration 010) |
| `labels` | JSONB | NULL | 청크 라벨 (Migration 013) |
...
**데이터 흐름:**
- **쓰기:** `POST /api/storage/audio/chunk` (multipart/form-data) — WAV 청크 업로드 시 Storage + DB 동시 저장
- **읽기:** `GET /api/storage/audio/chunks/:sessionId` — 세션별 청크 목록 조회
...
sessions ──────< billable_units ──────< delivery_records
    ...
    ├──< transcripts
    │
    ├──< session_chunks        (WAV 물리 청크, 009~013)
    │
    ├──< transcript_chunks     (STT 텍스트 + 오디오 통계, 011~012)
    │
    ├──< error_logs
    │
    └──< funnel_events
...
모든 Migration 파일(001~013)은 DB에 적용되어 있음. 아래는 테이블이 생성되었으나 **현재 API 코드에서 참조하지 않는** 테이블 목록.

- **Migration 005** (`005_asset_ledger.sql`): `daily_asset_stats`, `monthly_asset_stats`, `campaigns(v2)`, `campaign_progress` — 테이블 생성됨, 코드 미사용. `user_asset_ledger`와 `user_asset_ledger.unit_type`은 **v1.5 기준 활성 사용 중**.
- **Migration 006** (`006_device_event_units.sql`): `device_event_units`, `device_event_stats` — 테이블 생성됨, 코드 미사용. `user_asset_ledger.unit_type` 컬럼 추가는 활성 반영됨.
- **Migration 008** (`008_delivery_records_and_ledger_update.sql`): `delivery_records` 신규 (활성), `user_asset_ledger` 제약조건 확장 (META_EVENT_BASE + voided, 활성), `export_jobs.status` 확장 (delivered, 활성), `sessions.has_diarization` 추가 (활성).
